    ...
    if (type == "occasion") {
        getProductsByOccasion(tag).then((products) => {
            ...
            products.forEach(element => {
                document.getElementById("productPane").innerHTML += "<div class=\"product\"><img class=\"productImage\" src=\"" + element.imageLocation + "\" /><br />" + element.score.toString() + "/10 for " + tag + " " + element.productName + "<br />" + element.productDescription + "<br />" + element.comment + "<a href=\"showProduct.html?productID=" + element.productID + "\"</div>";
            });
        });
    } else {
